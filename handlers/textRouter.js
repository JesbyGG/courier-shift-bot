module.exports = function setupTextRouter(bot, services) {
  ...
  const TEXT_ROUTES = [
    ...
    { state: 'awaitingCarNumber', handler: async (ctx, s, text) => {
      ...
      if (result === 'done') await ctx.replyWithHTML(`${pe('✅')} Готово.`, getMenuForRole(ctx.from.id));
    }},
    ...
    { match: isMileageButton, handler: async (ctx) => {
      ...
      if (res.status === 'access_denied') await ctx.replyWithHTML(`${pe('❌')} Эта функция доступна только курьерам.`, getMenuForRole(ctx.from.id));
      else if (res.status === 'pedestrian_no_mileage') await ctx.replyWithHTML(`${pe('🚶')} Пешим курьерам пробег не требуется.`, getMenuForRole(ctx.from.id));
    ...
    { button: BUTTONS.cashCheck, legacy: ['Деньги к сдаче', '💵 Наличные'], handler: async (ctx) => {
      ...
      if (res.status === 'access_denied') await ctx.replyWithHTML(`${pe('❌')} Эта функция доступна только курьерам.`, getMenuForRole(ctx.from.id));
      else if (res.status === 'no_debt') await ctx.replyWithHTML(`${pe('✅')} Долгов нет — все деньги сданы.`, getMenuForRole(ctx.from.id));
      else if (res.status === 'already_submitted') await ctx.replyWithHTML(`${pe('⏳')} Вы уже отметили сдачу. Ожидайте подтверждения логиста.`, getMenuForRole(ctx.from.id));
    }},
    ...
    { button: BUTTONS.cashCollect, handler: async (ctx) => {
      ...
      if (res.status === 'access_denied') {
        await ctx.replyWithHTML(`${pe('❌')} Эта функция доступна только логистам.`, getMenuForRole(ctx.from.id));
        ...
        await ctx.replyWithHTML(`${pe('❌')} В этом магазине приём наличных не предусмотрен.`, getMenuForRole(ctx.from.id));
      } else if (res.status === 'no_debt') {
        await ctx.replyWithHTML(`${pe('✅')} Долгов нет — все деньги сданы.`, getMenuForRole(ctx.from.id));
      }
    }},
    ...
    { button: BUTTONS.openShop, handler: async (ctx) => {
      ...
      if (res.status === 'access_denied') await ctx.replyWithHTML(`${pe('❌')} Эта функция доступна только логистам.`, getMenuForRole(ctx.from.id));
      ...
      else if (res.status === 'ok') await ctx.replyWithHTML(`${pe('✅')} Уведомление отправлено: <b>${esc(res.workplace)} — ОТКРЫТ</b> ${pe('✅')}`, getMenuForRole(ctx.from.id));
    }},

    // 4) Меню настроек
    { button: BUTTONS.settings, legacy: ['Настройки'], handler: async (ctx, s, text, id) => ctx.replyWithHTML(`${pe('⚙️')} <b>Настройки</b>`, getSettingsMenuForRole(id)) },
    { button: BUTTONS.profile, legacy: ['Профиль'], handler: async (ctx, s, text, id) => ctx.replyWithHTML(`${pe('✏️')} <b>Профиль</b>`, getProfileMenuForRole(ctx.from.id)) },
    { button: BUTTONS.backToSettings, handler: async (ctx, s, text, id) => ctx.replyWithHTML(`${pe('⚙️')} <b>Настройки</b>`, getSettingsMenuForRole(id)) },
