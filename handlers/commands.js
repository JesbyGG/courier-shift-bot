module.exports = function setupCommands(bot, services) {
  ...
  bot.command("cancel", async (ctx) => {
    ...
    if (res.status === "mileage_processing")
      await ctx.replyWithHTML(
        "📸 Обработка фото пробега продолжается... результат придёт в новый чат.",
        getMenuForRole(ctx.from.id),
      );
    else if (res.status === "back_to_menu")
      await ctx.replyWithHTML(res.message, getMenuForRole(ctx.from.id));
  });
  ...
  bot.action("role_courier", async (ctx) => {
    ...
    if (!state?.awaitingRoleChoice) {
      await ctx.replyWithHTML(
        "⚠️ Выбор роли устарел. Нажмите /start.",
