module.exports = function setupCourier(bot, services) {
  ...
  bot.on("photo", async (ctx) => {
    ...
    if (state?.awaitingReconciliationPhoto) {
      ...
      if (res?.status === "photos_sent_terminal") {
        await ctx.replyWithHTML(
          `✅ Все фото (2 шт.) отправлены${res.ocrWarning}`,
          getMenuForRole(ctx.from.id),
        );
        ...
        await ctx.replyWithHTML(
          `✅ Все фото (${res.total} шт.) отправлены${res.ocrWarning}`,
          getMenuForRole(ctx.from.id),
        );
        if (res.postRes?.status === "partial_error") {
          await ctx.replyWithHTML(
    ...
    if (!state?.awaitingMileagePhoto) {
      await ctx.replyWithHTML(
        `⚠️ Сначала нажмите «${BUTTONS.mileage}»`,
        getMenuForRole(ctx.from.id),
      );
      return;
    }
  ...
  bot.action("confirm_switch_user", async (ctx) => {
    ...
    if (!swState?.awaitingSwitchUser) {
      await ctx.answerCbQuery(
        "⚠️ Сначала нажмите «Поменять сотрудника» в настройках.",
        { show_alert: true },
      );
      return;
    }
  ...
  bot.action("route_sheet_done", async (ctx) => {
    ...
    await withState(ctx.from.id, async (state) => {
      if (state?.awaitingReconciliationPhoto) {
        ...
        if (sent > 0) {
          try {
            ...
            safeLog.error(
              "route_sheet_done: failed to forward partial reconciliation photo",
              e.message,
            );
          }
        }
      ...
      clearState(ctx.from.id);
      const curSheets = Number(
        getUserField(ctx.from.id, "routeSheetsSubmitted") || 0,
      );
      setUserField(ctx.from.id, "routeSheetsSubmitted", curSheets + 1);
      await ctx.replyWithHTML(
        "✅ Маршрутный лист завершён\n\nСпасибо!",
        courierMainMenu(ctx.from.id),
      );
    });
  });
  ...
  bot.action("cash_submit_yes", async (ctx) => {
    ...
    const workplace =
      ...
      getUserField(telegramId, "workplace") ||
      "не указано";
    ...
    await notifyLogistsAboutSelfClearance(
      telegramId,
      courierFio,
      amount,
      formatted,
  ...
  bot.action("retry_mileage_photo", async (ctx) => {
    ...
    await ctx.replyWithHTML(
      "📷 Отправьте фото пробега крупным планом",
      skipMileageKeyboard(),
    );
  });
  ...
  bot.action("replace_mileage_start", async (ctx) => {
    ...
    if (res.status === "not_found")
      ...
      await ctx.replyWithHTML(
        "⚠️ Не удалось подготовить замену\n\nПопробуйте ещё раз.",
        getMenuForRole(ctx.from.id),
      );
  });
  ...
  bot.action("replace_start", async (ctx) => {
    ...
    if (res.status === "replaced") {
      await ctx.replyWithHTML(
        `🟢 <b>Старт смены</b> заменён\n\n⏰ <code>${esc(res.timeValue)}</code>`,
        getMenuForRole(ctx.from.id),
      );
  ...
  bot.action("confirm_skip_mileage", async (ctx) => {
    ...
    clearState(ctx.from.id);
    safeLog.log("пропуск пробега подтверждён");
  ...
  bot.action("edit_mileage", async (ctx) => {
    ...
    await ctx.replyWithHTML(
      "✏️ Ввод пробега\n──────────────\n\nВведите пробег цифрами или загрузите фото повторно.",
      manualMileageKeyboard(),
    );
  });
  ...
  bot.action("edit_time", async (ctx) => {
    ...
    if (
      !state?.awaitingTimeChange ||
      !state?.courierRow ||
      !state?.day ||
      !state?.stage
    ) {
      await ctx.replyWithHTML(
        `⚠️ Сначала нажмите «${BUTTONS.punchTime}»`,
        getMenuForRole(ctx.from.id),
      );
      return;
    }
