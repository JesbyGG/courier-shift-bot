...
async function sendPhotoToChat(
  ...
  fileId,
  caption,
  { envChatId, envThreadId, parseMode, fallbackChatId } = {},
) {
...
async function sendMediaGroupToChat(
  ctx,
  items,
  { envChatId, envThreadId, parseMode, fallbackChatId } = {},
) {
  ...
  if (!chatId) {
    safeLog.log(
      `${envChatId || "chatId"} is empty, media group is not forwarded`,
    );
    return null;
  }
  ...
  try {
    ...
    try {
      ...
      await ctx.telegram
        .deleteMessage(chatId, cleanupMsg.message_id)
        .catch(() => {});
    } catch (_) {}
...
function forwardPhoto(ctx, fileId, caption, destinationKey) {
  ...
  return sendPhotoToChat(ctx, fileId, caption, {
    ...
    fallbackChatId: dest.fallbackEnvChatId
      ? process.env[dest.fallbackEnvChatId]
      : undefined,
  });
}

const sendPhotoToWorkChat = (ctx, fileId, caption) =>
  forwardPhoto(ctx, fileId, caption, "work");
const sendPhotoToRouteSheetChat = (ctx, fileId, caption) =>
  forwardPhoto(ctx, fileId, caption, "routeSheet");
const sendPhotoToReconciliationChat = (ctx, fileId, caption) =>
  forwardPhoto(ctx, fileId, caption, "reconciliation");
...
const sendMediaGroupToReconciliationChat = (ctx, items) =>
  forwardMediaGroup(ctx, items, "reconciliation");
