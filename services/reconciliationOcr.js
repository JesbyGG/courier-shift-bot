...
function extractCashFromGemini(text) {
  ...
  if (cashMatch) {
    ...
    if (amount !== null && amount === 0)
      return { amount: 0, totalOrders, valid: false, reason: "cash_zero" };
    if (amount !== null && amount >= 1)
      return { amount, totalOrders, valid: true, reason: "ok" };
  }

  // 2) Legacy: "Наличные 5 / 17 777,18" or "Наличные 2/16 451,96"
  const legacyMatch = normalized.match(
    /(?:налич[а-яa-z]*)\s+\d{1,3}\s*\/\s*([\d\s.,]+)/i,
  );
  if (legacyMatch) {
  ...
  if (!hasStructFields) {
    ...
    if (plainAmount !== null && plainAmount === 0)
      return { amount: 0, totalOrders, valid: false, reason: "cash_zero" };
    if (plainAmount !== null && plainAmount >= 1)
      return { amount: plainAmount, totalOrders, valid: true, reason: "ok" };
  }
...
async function recognizeReconciliationCashSafe(ctx, fileId, label) {
  try {
    ...
    safeLog.error(
      "reconciliation OCR safe fallback",
      label || "",
      error.message || error,
    );
...
async function recognizeReconciliationCash(ctx, fileId) {
  try {
    ...
    if (isGeminiOcrEnabled()) {
      ...
      if (geminiText) {
        safeLog.log("сверка OCR raw text:", geminiText);
        const parsed = extractCashFromGemini(geminiText);
        safeLog.log(
          "сверка OCR:",
          JSON.stringify({
            cashValid: parsed.valid,
            cashReason: parsed.reason,
            cashAmount: parsed.amount,
        ...
        safeLog.log("сверка OCR: Gemini OCR вернул пустой текст");
      }
    }

    safeLog.log("сверка OCR: Gemini OCR не дал результата");
    return {
      amount: 0,
      totalOrders: 0,
      valid: false,
    ...
    safeLog.error(
      "reconciliation cash recognition error",
      error.message || error,
    );
    return {
      amount: 0,
