...
### Phase D — исправление часового пояса

- **Исправлен `getDateInTimezone`:** больше не создаёт `Date` в системном
  поясе сервера. Теперь возвращает части даты/времени в целевом поясе.
- **`getCurrentDateInfo`** возвращает `{ year, month, day, hour, minute, ... }`
  напрямую из `Intl.DateTimeFormat`, что исключает сдвиги UTC/CEST.
- **`roundTimeToHalfHour`** теперь принимает `{ hour, minute }` из целевого
  пояса вместо `Date.getHours()` (который возвращал системное время).
- Обновлены `punchTime`, `replaceTime`, `punchTimeFlow` — используют
  часовой пояс `APP_TIMEZONE`, а не системный.
- На VPS установлен системный пояс `Europe/Moscow`.
- Добавлены тесты на корректность часового пояса.
