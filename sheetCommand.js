...
function registerSheetCommand(bot, options = {}) {
  ...
  bot.command("sheet", async (ctx) => {
    ...
    if (adminIds.length === 0) {
      await ctx.replyWithHTML(
        "⛔ Команда отключена: не настроен <code>ADMIN_IDS</code> в .env.",
      );
      return;
    }
    ...
    if (!subcommand || subcommand === "list" || subcommand === "список") {
      ...
      for (const workplace of workplaces) {
        ...
        if (!currentId && active.noSheetForMonth) {
          message +=
            "   ⚠️ На активный месяц таблица не привязана — запись блокируется\n";
        }

        if (!active.isMonthly) {
          message +=
            "   ⚠️ Рекомендуется задать <code>active</code> и <code>next</code> вручную\n";
        }
      ...
      message += "Команды:\n";
      message +=
        "<code>/sheet east active URL</code> — добавить и сделать активной\n";
      message +=
        "<code>/sheet east next URL</code> — добавить на следующий месяц\n";
    ...
    if (subcommand === "reset") {
                ...
                [
                  {
                    ...
                    callback_data: "sheet_reset_all_confirm",
                  },
                ],
      ...
      if (!workplace) {
        await ctx.replyWithHTML(
          "❌ Укажите магазин: <code>/sheet reset east active</code> или <code>/sheet reset center next</code> (или <code>/sheet reset all</code>).",
        );
        return;
      }
      ...
      if (resetTarget === "all" || resetTarget === "все") {
        ...
        await ctx.replyWithHTML(
          `✅ Все привязки для <b>${esc(workplace)}</b> сброшены.`,
        );
        return;
      }
      ...
      if (!slot) {
        await ctx.replyWithHTML(
          "❌ Укажите режим: <code>active</code>, <code>next</code> или <code>YYYY-MM</code>.",
        );
        return;
      }
      ...
      await ctx.replyWithHTML(
        `✅ Привязка для <b>${esc(workplace)}</b> (${esc(slot.label)}: <code>${esc(slot.monthKey)}</code>) удалена.`,
      );
      return;
    }
    ...
    await ctx.replyWithHTML(
      `⏳ Проверяю таблицу для <b>${esc(workplace)}</b> (режим: ${esc(modeText)})...`,
    );
    ...
    try {
      ...
      await ctx.replyWithHTML(message);
      safeLog.log("sheet changed", workplace, monthKey, sheetId, result.title);
    } catch (error) {
      safeLog.error("sheet command error", error);
      await ctx.replyWithHTML(
        "⚠️ Произошла ошибка при проверке таблицы.\nПопробуйте ещё раз.",
      );
    }
  });
  ...
  bot.action("sheet_reset_all_confirm", async (ctx) => {
    ...
    await ctx.editMessageText("✅ Все привязки для всех магазинов сброшены.", {
      parse_mode: "HTML",
    });
  });
}
